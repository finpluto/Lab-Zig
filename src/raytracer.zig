...
const math = std.math;
const aegleseeker = @import("aegleseeker");
...
pub fn main() !void {
    ...
    _ = aegleseeker.as_init_scene(screen_height, screen_width);
    _ = aegleseeker.as_draw_to_pixel_buf(pixel_buf.ptr);
    ...
    mainLoop: while (true) {
        ...
        _ = aegleseeker.as_camera_yaw(yaw);
        _ = aegleseeker.as_camera_ztranslate(z_translate);
        _ = aegleseeker.as_light_position_offset(light_x, light_y, light_z);
        _ = aegleseeker.as_draw_to_pixel_buf(pixel_buf.ptr);
        try texture.update(pixel_buf, @sizeOf(u8) * 4 * screen_width, null);
        try renderer.clear();
